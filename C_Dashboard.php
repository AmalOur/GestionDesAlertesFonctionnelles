<?php
    ...
    $stmtContrat = $pdo->prepare("SELECT * FROM contrat WHERE DATEDIFF(date_fin, CURDATE()) <= 15");
    $stmtContrat->execute();
    $contrats = $stmtContrat->fetchAll(PDO::FETCH_ASSOC);

    $stmtCertificat = $pdo->prepare("SELECT * FROM certificat_ssl WHERE DATEDIFF(date_fin, CURDATE()) <= 15");
    $stmtCertificat->execute();
    $certificats = $stmtCertificat->fetchAll(PDO::FETCH_ASSOC);

    $stmtContratMaintenance = $pdo->prepare("SELECT m.*, c.application_source FROM details_maintenance m INNER JOIN contrat c ON m.id_contrat = c.id_contrat WHERE DATEDIFF(c.date_fin, CURDATE()) <= 15");
        ...
        <div class="main-header">
            ...
            <div class="content">
                ...
                <div class="vertical-tabs">
                    ...
                    <a href="C_DetailContrat.php">Contrat</a>
                </div>
                </div>
